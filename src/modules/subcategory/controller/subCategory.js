...
export const deleteSubCategory = async (req, res, next) => {
  const { subCategoryId } = req.params;
  const deleteCategory = await subCategoryModel.findByIdAndDelete(subCategoryId);
  if (!deleteCategory) {
    return next(new Error("Subcategory not found"), {
      cause: StatusCodes.CONFLICT,
    });
...
export const searchBynameSubCat=async(req,res,next)=>{
    ...
    const findCategory=await subCategoryModel.find({name:{$regex: new RegExp(`${searchKey}`,"i")}})
